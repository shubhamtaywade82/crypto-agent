export function checkHtfConflict(obs, htf = '1h') {
...
export function checkEarlyFailure(obs, maxBars = 3) {
    return obs.outcome.firstHit === 'stop_first' && obs.outcome.timeToFirstHitBars <= maxBars;
}
export function checkInvalidatedZone(obs) {
    if ('isInvalidated' in obs.outcome) {
        return obs.outcome.isInvalidated;
    }
    return false;
}
export function classifyNegativeEvidence(obs, htf = '1h', maxBarsForEarlyFail = 3) {
    const eventDir = obs.event.direction;
 ...
 */
export function evaluateNegativeEvidenceImpact(observations, htf = '1h') {
...
//# sourceMappingURL=negative-evidence.js.map