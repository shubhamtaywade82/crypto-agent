...
function generateRealisticCandles(count, basePrice = 60000) {
    const candles = [];
    ...
    for (let i = 0; i < count; i++) {
        ...
        let step = 0;
        if (cycle < 10)
            step = 15; // initial climb
        else if (cycle < 16)
            step = -20; // retracement establishing swing high
        else if (cycle < 26)
            step = 30; // powerful expansion breaking the swing high (BOS + OB)
        else if (cycle < 32)
            step = -15; // pullback
        else
 ...
 */
export function runResearchCli(symbol = 'BTCUSDT') {
