...
import { DETECTABLE_EVENT_TYPES } from './types.js';
 ...
 */
function verifiedResult(name, output, startedAt) {
    return {
...
function failedResult(name, error, startedAt) {
 ...
 */
function getEvents(ctx, eventType) {
 ...
 */
export function createResearchTools(context) {
    const catalogue = new ToolkitCatalogue();
    /* -- 1. discovery ------------------------------------------------- */
    catalogue.place({
        handle: 'list_event_detectors',
        caption: 'List the deterministic market-event detectors available for research on this dataset.',
        argsShape: z.object({}),
        resourceClass: 'local-cpu',
        effects: 'pure',
        grantLevel: 'auto',
    ...
    catalogue.place({
        ...
        caption: 'Run a deterministic market-event detector against the current dataset and return the detected events with full provenance (origin index, available-at index, direction, timeline).',
        argsShape: z.object({
    ...
    catalogue.place({
        ...
        caption: 'Calculate the deterministic market context (trend regime, volatility regime, ATR, session, optional HTF context) at a specified candle index. Use this to qualify event observations with the regime they occurred in.',
        argsShape: z.object({
        ...
        invoke: async (args) => {
            ...
            try {
                ...
                const snapshot = extractContextSnapshot(context.candles, index, toHtfCandlesMap(context));
                const candle = context.candles[index];
                return verifiedResult('get_market_context', {
                    symbol: context.symbol,
                    timeframe: context.timeframe,
                    candleIndex: index,
    ...
    catalogue.place({
        handle: 'run_study',
        caption: 'Run the deterministic universal market-event research study across all event types. Returns per-event-type sample sizes, reach rates (1R/2R/3R), matched-control baseline comparison, Wilson confidence intervals, and Benjamini-Hochberg multiple-testing adjustment.',
        argsShape: z.object({
    ...
    catalogue.place({
        ...
        caption: 'Run deterministic empirical research for one specific event type. Returns the same shape as run_study but scoped to a single detector, so the model can drill into one hypothesis at a time.',
        argsShape: z.object({
            eventType: z.enum(DETECTABLE_EVENT_TYPES),
            horizonCandles: z.number().int().positive().default(24),
        }),
        ...
        invoke: async (args) => {
            ...
            try {
                ...
                return verifiedResult('run_event_study', {
                    symbol: context.symbol,
                    timeframe: context.timeframe,
                    eventType: args.eventType,
                    horizonCandles: args.horizonCandles,
    ...
    catalogue.place({
        ...
        invoke: async (args) => {
            ...
            try {
                ...
                const pair = analyzeEventPairInteraction(observationsA, observationsB, args.targetMetric, args.maxBarGap);
                const anchor = analyzeAnchorInteraction(observationsA, observationsB.map((o) => o.event), {
                ...
                return verifiedResult('evaluate_interaction', {
                    symbol: context.symbol,
                    timeframe: context.timeframe,
                    horizonCandles: args.horizonCandles,
                    targetMetric: args.targetMetric,
    ...
    catalogue.place({
        ...
        invoke: async (args) => {
            ...
            try {
                ...
                return verifiedResult('evaluate_negative_evidence', {
                    symbol: context.symbol,
                    timeframe: context.timeframe,
                    eventType: args.eventType,
                    htf: args.htf,
    ...
    catalogue.place({
        ...
        caption: 'Return a compact summary of the bound dataset: symbol, timeframe, candle count, first/last timestamp, and the list of event detectors that produced non-zero event counts. Use this at the start of every run to ground the model in the dataset it is researching.',
        argsShape: z.object({}),
        resourceClass: 'local-cpu',
        effects: 'pure',
        grantLevel: 'auto',
        ...
        invoke: async () => {
            ...
            return verifiedResult('dataset_summary', {
                symbol: context.symbol,
                timeframe: context.timeframe,
                ...
                htfCandles: context.htfCandles
                    ? Object.fromEntries(Object.entries(context.htfCandles).map(([tf, arr]) => [
                        tf,
                        arr ? arr.length : 0,
                    ]))
                    : null,
     ...
     */
    function buildObservations(eventType, horizonCandles) {
        const study = runObservationStudy(context.candles, {
            symbol: context.symbol,
            timeframe: context.timeframe,
            horizonCandles,
 ...
 */
export async function invokeToolDirect(catalogue, handle, args) {
    ...
    if (!parsed.success) {
        const issues = parsed.error && typeof parsed.error.issues === 'object'
            ? parsed.error.issues
            : parsed.error;
        return {
            toolCallId: crypto.randomUUID(),
